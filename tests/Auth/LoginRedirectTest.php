...
final class LoginRedirectTest extends WebTestCase
{
    ...
    private const STAFF_EMAIL = 'login-redirect-staff@example.com';
    ...
    protected function setUp(): void
    {
        ...
        $staff = (new User())
            ->setEmail(self::STAFF_EMAIL)
            ->setFirstName('Sta')
            ->setLastName('Ff')
            ->setRoles(['ROLE_SECTION_TOOLS'])
            ->setCreatedAt(new \DateTimeImmutable())
            ->setProfileComplete(true)
            ->setVerified(true);
        $staff->setPassword($hasher->hashPassword($staff, self::PASSWORD));
    ...
    public function testSectionStaffLoginRedirectsToDashboard(): void
    {
        $this->client->request('GET', self::LOGIN_PATH);
        $this->client->submitForm(self::SUBMIT_BUTTON, [
            '_username' => self::STAFF_EMAIL,
            '_password' => self::PASSWORD,
        ]);
        ...
        $location = (string) $this->client->getResponse()->headers->get('Location');
        self::assertStringContainsString('/'.$this->adminPrefix.'/admin', $location);
        self::assertStringNotContainsString('/admin/outils', $location);
    }
